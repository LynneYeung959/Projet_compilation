...
node_t make_node_string(node_nature nature);
node_t make_node_intval(node_nature nature, int32_t val);
node_t make_node_bool(int n);
/* A completer */
...
/* la ligne suivante est pour tester les identificateurs*/
...
paramprint: 
	...
	| TOK_STRING
	{
		$$ = make_node_string(NODE_STRINGVAL);
	}
	;
...
node_t make_node(node_nature nature, int nops, ...) {
	...
	{
		node -> opr = malloc(nops * sizeof(node_s));
		//parcourir va_list opr;
		va_start(ap,nops);
